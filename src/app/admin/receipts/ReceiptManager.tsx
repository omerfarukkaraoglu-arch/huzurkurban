            ...
            {filteredRegistrations.map((reg: any) => (
              <tr key={reg.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-6 py-4">
                  ...
                  <div className="text-xs text-slate-700 font-bold">{reg.phone}</div>
                </td>
          ...
          <div className="w-[210mm] h-[148.5mm] mx-auto p-10 bg-white relative">
            <div className="flex justify-between items-start mb-6 border-b-2 border-slate-100 pb-6">
              <div>
                ...
                <p className="text-slate-800 font-black uppercase text-xs tracking-widest leading-none">Resmi Ödeme Makbuzu</p>
              </div>
              <div className="text-right">
                <div className="text-slate-800 font-black text-lg">NO: #{(selectedReg.id).slice(-6).toUpperCase()}</div>
                <div className="text-slate-800 text-sm font-bold">Tarih: {new Date().toLocaleDateString('tr-TR')}</div>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-10 mb-8">
              <div className="border-l-4 border-emerald-500 pl-4">
                <h3 className="text-[10px] font-black text-slate-700 uppercase mb-1">Makbuz Sahibi</h3>
                ...
                <div className="text-slate-800 text-xs leading-relaxed max-w-[280px] font-medium">{selectedReg.address}</div>
              </div>
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-slate-700 text-xs font-black uppercase">Grup:</span>
                  <span className="text-slate-900 font-bold text-sm">{selectedReg.group}</span>
                </div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-slate-700 text-xs font-black uppercase">Hisse:</span>
            ...
            <div className="text-slate-800 text-xs mb-8 leading-relaxed bg-slate-50 p-3 rounded-lg border border-slate-200 font-medium">
            ...
            <div className="flex justify-between items-end absolute bottom-10 left-10 right-10">
              <div className="text-center">
                <p className="text-[10px] text-slate-700 uppercase font-black mb-12">Müşteri İmza</p>
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] text-slate-700 uppercase font-black mb-8">Yetkili İmza / Kaşe</p>
                <div className="w-40 border-b border-slate-300"></div>
              </div>
            </div>
          </div>
